...
import newsRouter from "./routes/news/news.routes.js";
import gardenRouter from "./routes/plants_recommendation/plants_recommendation_route.js"; 
import advisoryRoutes from "./routes/fertilizer_advisery/fertilizer_advisery.js";
import harvestRoutes from "./routes/harvest_advisery/harvest_advisery.js";
...
app.use("/api/v1/homegarden", gardenRouter);
...
app.use((err, req, res, next) => {
  ...
  // Multer / file upload validation errors should be 400.
  const isMulterError = err?.name === 'MulterError';
  const isFileTypeError = typeof err?.message === 'string' && err.message.toLowerCase().includes('invalid file type');
  const status = isMulterError || isFileTypeError ? 400 : (err.statusCode || 500);

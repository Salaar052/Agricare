...
export async function verifyJwt(req, res, next) {
  try {
    ...
    // Block access for unverified (non-admin) users, but allow auth check + logout.
    if (!user.isAdmin && user.isEmailVerified === false) {
      const path = req.originalUrl || req.path || "";
      const allowed = path.includes("/api/v1/auth/check") || path.includes("/api/v1/auth/logout");

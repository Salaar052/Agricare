...
class MarketplaceService {
  ...
  Future<Map<String, dynamic>> editMyProfile({
    ...
    try {
      ...
      if (whatsappNumber != null) body['whatsapp_number'] = whatsappNumber.trim();
  ...
  Future<void> addToSavedItems(String itemId, {bool showToast = true}) async {
    try {
      ...
      final response = await _makeRequest('POST', '/marketplace/saved-items', body: {'itemId': itemId});
  ...
  Future<void> removeFromSavedItems(String itemId, {bool showToast = true}) async {
    try {
      ...
      final response = await _makeRequest('DELETE', '/marketplace/saved-items/$itemId');
      _parseResponse(response);
  ...
  /// Get public seller profile with stats + listings
/// GET /marketplace/seller/:sellerId/profile
/// Returns: { profile, stats, listings }
  Future<Map<String, dynamic>> getPublicSellerProfile(String sellerId) async {
  try {
    print('🏪 Fetching public seller profile: $sellerId');
    ...
    final response = await _makeRequest(
      'GET',
      '/marketplace/seller/$sellerId/profile',
    );
    final data = _parseResponse(response);

    print('✅ Public seller profile fetched');
    return data['data'] ?? {};
  } catch (e) {
    print('❌ Fetch public seller profile error: $e');
    rethrow;
  }
  }
  // ==================== MY LISTINGS ====================
  ...
  /// Pending listing requests (awaiting super admin approval)
  Future<Map<String, dynamic>> adminGetPendingListings({
    int page = 1,
    int limit = 100,
  }) async {
    try {
      final uri = Uri.parse('$baseUrl/marketplace/admin/pending-items').replace(
        queryParameters: {
          'page': page.toString(),
          'limit': limit.toString(),
        },
      ...
      final response =
          await http.get(uri, headers: _getHeaders()).timeout(const Duration(seconds: 30));
      final data = _parseResponse(response);
      return data['data'] ?? {};
    } catch (e) {
      print('❌ Admin pending listings error: $e');
      rethrow;
    }
  }

  /// All marketplace listings for super admin (any status; optional status filter)
  Future<Map<String, dynamic>> adminGetAllListings({
    int page = 1,
    int limit = 100,
    String status = 'all',
  }) async {
    try {
      ...
      final uri = Uri.parse('$baseUrl/marketplace/admin/all-listings')
      ...
      print('❌ Admin all listings error: $e');
      rethrow;
    }
  }

  Future<void> adminApproveListing(String productId) async {
    try {
      if (productId.isEmpty) throw Exception('Product ID is required');
      final response = await _makeRequest(
        'PUT',
        '/marketplace/admin/items/$productId/approve',
      );
      _parseResponse(response);
  ...
  Future<void> adminRejectListing(String productId, {String? rejectionReason}) async {
    try {
      if (productId.isEmpty) throw Exception('Product ID is required');
      final response = await _makeRequest(
        'PUT',
        '/marketplace/admin/items/$productId/reject',
